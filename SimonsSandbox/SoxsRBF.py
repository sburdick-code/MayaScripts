...
class SoxsRBFNode(om.MPxNode):
    ...
    def compute(self, plug, data):
        ...
        # check if the new position still matches the old, if it does, no need to recalculate
        ...
        # Set the old position, so if it doesn't change, no need to recalculate!
        ...
        # Calculate RBF
        solve_result = self.evaluate_rbf(position, in_matrix, weight_matrix, eq_index)
        print(solve_result)

        # Set outputs
        output_handle = data.outputValue(SoxsRBFNode.output_t_obj)
        out_x_t = output_handle.child(SoxsRBFNode.out_x_t_obj)
        out_y_t = output_handle.child(SoxsRBFNode.out_y_t_obj)
        out_z_t = output_handle.child(SoxsRBFNode.out_z_t_obj)

        output_handle = data.outputValue(SoxsRBFNode.output_r_obj)
        out_x_r = output_handle.child(SoxsRBFNode.out_x_r_obj)
        out_y_r = output_handle.child(SoxsRBFNode.out_y_r_obj)
        out_z_r = output_handle.child(SoxsRBFNode.out_z_r_obj)

        output_handle = data.outputValue(SoxsRBFNode.output_s_obj)
        out_x_s = output_handle.child(SoxsRBFNode.out_x_s_obj)
        out_y_s = output_handle.child(SoxsRBFNode.out_y_s_obj)
        out_z_s = output_handle.child(SoxsRBFNode.out_z_s_obj)

        # Set out attributes
        if SoxsRBFNode.SETTINGS_ENUMS[eval_index][-4:] == "1Out":
            out_x_t.setDouble(solve_result[0])
            out_x_r.setMAngle(om.MAngle(solve_result[0], om.MAngle.kDegrees))
            out_x_s.setDouble(solve_result[0])

        if SoxsRBFNode.SETTINGS_ENUMS[eval_index][-4:] == "2Out":
            out_x_t.setDouble(solve_result[0])
            out_x_r.setMAngle(om.MAngle(solve_result[0], om.MAngle.kDegrees))
            out_x_s.setDouble(solve_result[0])
            out_y_t.setDouble(solve_result[1])
            out_y_r.setMAngle(om.MAngle(solve_result[1], om.MAngle.kDegrees))
            out_y_s.setDouble(solve_result[1])
        ...
        if SoxsRBFNode.SETTINGS_ENUMS[eval_index][-4:] == "3Out":
            ...
            out_y_r.setMAngle(om.MAngle(solve_result[1], om.MAngle.kDegrees))
            out_y_s.setDouble(solve_result[1])
            out_z_t.setDouble(solve_result[2])
            out_z_r.setMAngle(om.MAngle(solve_result[2], om.MAngle.kDegrees))
            out_z_s.setDouble(solve_result[2])
    ...
    def initialize(cls):
        ...
        # Translate
        cls.out_x_t_obj = nAttr.create(
            "OutputTranslateX", "outTX", om.MFnNumericData.kDouble, 0.0
        )
        nAttr.writable = False
        nAttr.storable = False

        cls.out_y_t_obj = nAttr.create(
            "OutputTranslateY", "outTY", om.MFnNumericData.kDouble, 0.0
        )
        nAttr.writable = False
        nAttr.storable = False

        cls.out_z_t_obj = nAttr.create(
            "OutputTranslateZ", "outTZ", om.MFnNumericData.kDouble, 0.0
        )
        nAttr.writable = False
        nAttr.storable = False

        cls.output_t_obj = cAttr.create("OutputTranslate", "outT")
        cAttr.writable = False
        cAttr.addChild(cls.out_x_t_obj)
        cAttr.addChild(cls.out_y_t_obj)
        cAttr.addChild(cls.out_z_t_obj)

        # Rotate
        cls.out_x_r_obj = uAttr.create(
            "OutputRotateX", "outRX", om.MFnUnitAttribute.kAngle, 0.0
        )
        nAttr.writable = False
        nAttr.storable = False

        cls.out_y_r_obj = uAttr.create(
            "OutputRotateY", "outRY", om.MFnUnitAttribute.kAngle, 0.0
        )
        nAttr.writable = False
        nAttr.storable = False

        cls.out_z_r_obj = uAttr.create(
            "OutputRotateZ", "outRZ", om.MFnUnitAttribute.kAngle, 0.0
        )
        nAttr.writable = False
        nAttr.storable = False

        cls.output_r_obj = cAttr.create("OutputRotate", "outR")
        cAttr.writable = False
        cAttr.addChild(cls.out_x_r_obj)
        cAttr.addChild(cls.out_y_r_obj)
        cAttr.addChild(cls.out_z_r_obj)

        # Scale
        cls.out_x_s_obj = nAttr.create(
            "OutputScaleX", "outSX", om.MFnNumericData.kDouble, 0.0
        )
        nAttr.writable = False
        nAttr.storable = False

        cls.out_y_s_obj = nAttr.create(
            "OutputScaleY", "outSY", om.MFnNumericData.kDouble, 0.0
        )
        nAttr.writable = False
        nAttr.storable = False

        cls.out_z_s_obj = nAttr.create(
            "OutputScaleZ", "outSZ", om.MFnNumericData.kDouble, 0.0
        )
        nAttr.writable = False
        nAttr.storable = False

        cls.output_s_obj = cAttr.create("OutputScale", "outS")
        cAttr.writable = False
        cAttr.addChild(cls.out_x_s_obj)
        cAttr.addChild(cls.out_y_s_obj)
        cAttr.addChild(cls.out_z_s_obj)
        ...
        cls.addAttribute(cls.output_t_obj)
        cls.addAttribute(cls.output_r_obj)
        cls.addAttribute(cls.output_s_obj)
