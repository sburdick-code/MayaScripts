...
class OutlinerWidget(MayaQWidgetDockableMixin, QtWidgets.QWidget):
    ...
    def workspace_ui_script(cls):
        # Add this command to the workspace control
    ...
    def add_to_workspace_control(self):
        ...
        if workspace_control:
            workspace_control_ptr = int(
                workspace_control
            )  # get a pointer of the workspace control name
            widget_ptr = int(getCppPointer(self)[0])  # get a pointer of self
    ...
    def showEvent(self, e):
        ...
        geometry = self.get_geometry_opt_var()
    ...
    def closeEvent(self, e):
        ...
        self.set_geometry_opt_var()

    def set_geometry_opt_var(self):
        geometry = self.saveGeometry()
        geometry_str = str(geometry.toHex().data().decode())

        cmds.optionVar(sv=(self.OUTLINER_GEOMETRY_OPT_VAR, geometry_str))

    def get_geometry_opt_var(self):
        if cmds.optionVar(exists=self.OUTLINER_GEOMETRY_OPT_VAR):
            hex_str_value = cmds.optionVar(q=self.OUTLINER_GEOMETRY_OPT_VAR)
            hex_byte_array = QtCore.QByteArray(hex_str_value.encode())

            return QtCore.QByteArray.fromHex(hex_byte_array)

        return QtCore.QByteArray()

    @classmethod
    def clear_opt_var(cls):
        cmds.optionVar(remove=cls.OUTLINER_GEOMETRY_OPT_VAR)
